...
exports.adminTrailService = exports.AdminTrailService = void 0;
const error_1 = require("../../middlewares/error");
const prisma_1 = require("../../services/prisma");
const audit_service_1 = require("../audit/audit.service");
const media_mappers_1 = require("../media/media.mappers");
...
const TRAIL_DIFFICULTY_SET = new Set(trail_schemas_1.TRAIL_DIFFICULTY_VALUES);
...
const ensureSlug = (input) => {
    const slug = slugify(input);
    if (slug.length === 0) {
        throw new error_1.HttpError(400, 'INVALID_SLUG', 'Unable to derive a valid slug for the trail');
    }
    return slug;
};
...
const parseDifficultyFilter = (value) => {
    if (!value) {
        return undefined;
    }
    ...
    for (const difficulty of normalized) {
        if (!TRAIL_DIFFICULTY_SET.has(difficulty)) {
            throw new error_1.HttpError(400, 'INVALID_DIFFICULTY', `Invalid difficulty "${difficulty}"`);
        }
        if (!result.includes(difficulty)) {
...
const parseNumericId = (value) => {
    ...
    if (typeof value === 'number') {
        if (!Number.isInteger(value) || value <= 0) {
            throw new error_1.HttpError(400, 'INVALID_IDENTIFIER', 'Identifier must be a positive integer');
        }
        return value;
    }
    const trimmed = value.trim();
...
const toTrailSummary = (trail) => ({
    ...
    media: trail.medias.map(media_mappers_1.toMediaSummary),
});
const toTrailDetail = (trail) => ({
...
class AdminTrailService {
    ...
    async ensureUniqueSlug(slug, excludeId) {
        ...
        if (existing) {
            throw new error_1.HttpError(409, 'TRAIL_SLUG_EXISTS', 'Trail with this slug already exists');
        }
    }
    async resolveState(stateId) {
        const state = await this.prismaClient.state.findUnique({ where: { id: stateId } });
        if (!state) {
            throw new error_1.HttpError(404, 'STATE_NOT_FOUND', 'State not found');
        }
        return state;
    }
    async resolveCity(cityId) {
        ...
        if (!city) {
            throw new error_1.HttpError(404, 'CITY_NOT_FOUND', 'City not found');
        }
        return city;
    }
    ...
    async createTrail(actor, input, context) {
        ...
        let resolvedStateId = input.stateId;
        let resolvedCityId = input.cityId;
        ...
        if (resolvedStateId !== undefined) {
            if (resolvedStateId === null) {
                if (resolvedCityId) {
                    throw new error_1.HttpError(400, 'CITY_STATE_MISMATCH', 'City cannot be associated without a valid state');
                }
            }
            else {
                ...
                if (resolvedCity && resolvedCity.stateId !== state.id) {
                    throw new error_1.HttpError(400, 'CITY_STATE_MISMATCH', 'City does not belong to the specified state');
                }
            }
        }
    ...
    async updateTrail(actor, id, body, context) {
        const roles = normalizeRoles(actor.roles);
        if (!(roles.has('ADMIN') || roles.has('EDITOR'))) {
            throw new error_1.HttpError(403, 'INSUFFICIENT_ROLE', 'User lacks required role');
        }
        ...
        if (body.distanceKm !== undefined) {
            updateData.distanceKm = (body.distanceKm);
        ...
        let resolvedStateId = existing.stateId ?? null;
        let resolvedCityId = existing.cityId ?? null;
        let currentCity = existing.city ? existing.city : null;
    ...
    async deleteTrail(actor, params, context) {
        const roles = normalizeRoles(actor.roles);
        if (!roles.has('ADMIN')) {
            throw new error_1.HttpError(403, 'INSUFFICIENT_ROLE', 'User lacks required role');
        }
...
exports.AdminTrailService = AdminTrailService;
exports.adminTrailService = new AdminTrailService();
